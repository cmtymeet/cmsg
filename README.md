...
Reopening currently requires the retained, authenticated pair MLS group. An already enrolled device can recover through ordered group catchup and private contact-history sync; creating a replacement group after losing that state needs a separate admission and recovery integration. Delayed resolution receipts update their exact archived introduction without changing the current contact gate, and still require current signer authorization when delivered.
