...
impl MemberIdentity {
    ...
    fn from_signer(community_id: &str, signer: SigningKey) -> Result<Self, Error> {
        ...
        Ok(Self { community_id: community_id.to_owned(), member_id, signer })
    }

    pub fn member_id(&self) -> &str { &self.member_id }
    pub fn community_id(&self) -> &str { &self.community_id }
    pub fn public_key(&self) -> [u8; 32] { self.signer.verifying_key().to_bytes() }
    ...
    pub fn authorize_device(
        ...
        if verifying.is_weak() { return Err(Error::Admission); }
        ...
        authorization.signature = BASE64URL_NOPAD.encode(
            &self.signer.sign(&authorization_bytes(&authorization)?).to_bytes(),
        );
        Ok(authorization)
    }
    ...
    pub fn restore(
        ...
        let secret = Zeroizing::new(<[u8; 32]>::try_from(plaintext.as_slice()).map_err(|_| Error::InvalidStore)?);
        let identity = Self::from_signer(community_id, SigningKey::from_bytes(&secret))
            .map_err(|_| Error::InvalidStore)?;
        if identity.member_id != expected_member_id { return Err(Error::InvalidStore); }
        Ok(identity)
    }
}
...
pub fn member_id_for_root(community_id: &str, root_public_key: &[u8; 32]) -> Result<String, Error> {
    ...
    let bytes = serde_json::to_vec(&serde_json::json!([
        "cmsg.member.v1", community_id, BASE64URL_NOPAD.encode(root_public_key)
...
pub fn verify_device_authorization(
    ...
    if member_id_for_root(community_id, &root)? != member_id { return Err(Error::Admission); }
    let device = decode32(&authorization.device_public_key)?;
    if VerifyingKey::from_bytes(&device).map_err(|_| Error::Admission)?.is_weak() {
        return Err(Error::Admission);
    }
    let bytes = BASE64URL_NOPAD.decode(authorization.signature.as_bytes()).map_err(|_| Error::Admission)?;
    let signature = Signature::from_slice(&bytes).map_err(|_| Error::Admission)?;
    VerifyingKey::from_bytes(&root).map_err(|_| Error::Admission)?
...
fn authorization_bytes(authorization: &DeviceAuthorization) -> Result<Vec<u8>, Error> {
    serde_json::to_vec(&serde_json::json!([
        "cmsg.device.v1", authorization.community_id, authorization.member_id,
        authorization.root_public_key, authorization.device_public_key,
        authorization.issued_at, authorization.expires_at,
    ])).map_err(|_| Error::Admission)
}

fn validate_community(community_id: &str) -> Result<(), Error> {
    if community_id.is_empty() || community_id.len() > 256
        || !community_id.bytes().all(|b| b.is_ascii_alphanumeric() || b"._:/-".contains(&b))
    { return Err(Error::Admission); }
    Ok(())
}
...
fn decode32(value: &str) -> Result<[u8; 32], Error> {
    if value.len() != 43 { return Err(Error::Admission); }
    BASE64URL_NOPAD.decode(value.as_bytes()).map_err(|_| Error::Admission)?
        .try_into().map_err(|_| Error::Admission)
}
...
fn identity_context(community: &str, member: &str, context: &[u8]) -> Result<[u8; 32], Error> {
    ...
    let bytes = serde_json::to_vec(&serde_json::json!([
        "cmsg.identity-store.v1", community, member, BASE64URL_NOPAD.encode(context)
    ])).map_err(|_| Error::InvalidStore)?;
