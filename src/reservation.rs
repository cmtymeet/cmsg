...
pub struct ReservationExpectation {
    pub community:[u8;32], pub owner:[u8;32], pub peer:[u8;32], pub role:u8,
    pub nonce:[u8;32], pub group:[u8;32], pub contact_policy_digest:[u8;32], pub history_digest:[u8;32],
    pub phase:u8, pub opened_at:u64, pub challenge:[u8;32],
}
#[derive(Clone,Debug,PartialEq,Eq,Serialize,Deserialize)]
#[serde(rename_all="camelCase",deny_unknown_fields)]
pub struct ReservationContext {pub now:u64,pub device_public_key:[u8;32],pub expected:ReservationExpectation}
#[derive(Clone,Debug,PartialEq,Eq,Serialize,Deserialize)]
#[serde(rename_all="camelCase",deny_unknown_fields)]
pub struct ReservationContexts {pub outgoing:ReservationContext,pub incoming:ReservationContext}
#[derive(Clone,Debug,PartialEq,Eq,Serialize,Deserialize)]
#[serde(rename_all="camelCase",deny_unknown_fields)]
...
pub trait ReservationVerifier {
    fn verify_remote(&mut self,evidence:&[u8],context:&ReservationContext)->Result<VerifiedReservation,Error>;
    fn verify_current_local(&mut self,evidence:&[u8],context:&ReservationContext)->Result<VerifiedReservation,Error>;
}
