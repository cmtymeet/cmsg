mod common;
use cmsg::{Clock, ContactDirective, ContactDirectiveKind as Kind, Error, FirstContactPolicy, Member, MemberIdentity};
...
fn device(root: &MemberIdentity, time: &Arc<Time>, expiry: u64) -> Member {
    ...
    member.bind_device_admission(grant, common::trust(),
        root.authorize_device(&key, 1, expiry).unwrap(), 100).unwrap();
    member
}
...
fn every_reopening_scope_is_authenticated_and_the_peer_cannot_impersonate_the_blocker() {
    ...
    let policy = FirstContactPolicy { response_deadline: 500, max_intro_bytes: 512 };
    let original = alice.sign_contact_directive(bob_root.member_id(), 2, &[9; 32], &[8; 32],
        Kind::FreshInitiative, &[7; 32], alice_root.member_id(), b"fresh-mls-group", Some(policy), None).unwrap();
    bob.verify_contact_directive(&original, alice_root.member_id()).unwrap();
    alice.verify_contact_directive(&original, alice_root.member_id()).unwrap();
    assert!(outsider.verify_contact_directive(&original, alice_root.member_id()).is_err());
    assert!(bob.verify_contact_directive(&original, bob_root.member_id()).is_err());
    ...
    let changes = [
        ...
        ("policy", serde_json::json!({"response_deadline":501,"max_intro_bytes":512})),
    ...
    for (field, value) in changes {
        ...
        assert!(bob.verify_contact_directive(&changed, alice_root.member_id()).is_err(), "{field}");
        assert_ne!(original.digest().unwrap(), changed.digest().unwrap(), "{field}");
    }
    let mut forged = bob.sign_contact_directive(alice_root.member_id(), 1, &[0; 32], &[0; 32],
        Kind::Block, &[0; 32], "", b"", None, None).unwrap();
    forged.owner_id = alice_root.member_id().to_owned();
    forged.peer_id = bob_root.member_id().to_owned();
    assert!(bob.verify_contact_directive(&forged, alice_root.member_id()).is_err());
...
fn block_expiry_is_signed_and_does_not_extend_expired_device_authority() {
    ...
    let bob = device(&bob_root, &clock, 1000);
    let directive = alice.sign_contact_directive(bob_root.member_id(), 1, &[0; 32], &[0; 32],
        Kind::Block, &[0; 32], "", b"", None, Some(200)).unwrap();
    bob.verify_contact_directive(&directive, alice_root.member_id()).unwrap();
    let mut changed = directive.clone();
    changed.until = None;
    assert!(bob.verify_contact_directive(&changed, alice_root.member_id()).is_err());
    ...
    assert!(bob.verify_contact_directive(&directive, alice_root.member_id()).is_err());
}
...
fn signers_reject_ambiguous_timer_chain_and_conversation_shapes() {
    ...
    for (revision, predecessor, until) in [
        (0, [0; 32], None), (1, [1; 32], None), (2, [0; 32], None),
        (1, [0; 32], Some(100)), (1, [0; 32], Some(u64::MAX)),
    ] {
        assert!(alice.sign_contact_directive(bob_root.member_id(), revision, &predecessor, &[0; 32],
            Kind::Block, &[0; 32], "", b"", None, until).is_err());
    }
    let policy = FirstContactPolicy { response_deadline: 500, max_intro_bytes: 512 };
    for (intro, group, candidate_policy, until) in [
        ...
        ([1; 32], b"group".as_slice(), Some(FirstContactPolicy { response_deadline: 100, ..policy }), None),
        ([1; 32], b"group".as_slice(), Some(FirstContactPolicy { max_intro_bytes: 0, ..policy }), None),
    ] {
        assert!(alice.sign_contact_directive(bob_root.member_id(), 1, &[0; 32], &[0; 32],
            Kind::FreshInitiative, &intro, alice_root.member_id(), group, candidate_policy, until).is_err());
    }
}
