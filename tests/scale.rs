...
fn hundred_members_exchange_rekey_and_exclude_a_removed_member() {
    ...
    let mut owner = Member::new().unwrap(); owner.create_group().unwrap();
    let mut peers: Vec<Member> = (0..99).map(|_| Member::new().unwrap()).collect();
    ...
    for peer in &mut peers { peer.join(&invitation.welcome).unwrap(); }
    let setup_ms = start.elapsed().as_millis();
    let start = Instant::now();
    let wire = owner.send(b"one hundred synthetic participants").unwrap();
    for peer in &mut peers {
        assert!(matches!(peer.receive(&wire).unwrap(), Received::Text(t) if t == "one hundred synthetic participants"));
    }
    let fanout_ms = start.elapsed().as_millis();
    let start = Instant::now();
    let commit = owner.remove(50).unwrap();
    for peer in &mut peers { let _ = peer.receive(&commit); }
    let wire = owner.send(b"remaining members only").unwrap();
    for (index, peer) in peers.iter_mut().enumerate() {
        if index == 49 { assert!(peer.receive(&wire).is_err()); }
        else { assert!(matches!(peer.receive(&wire).unwrap(), Received::Text(t) if t == "remaining members only")); }
    }
    let rekey_fanout_ms = start.elapsed().as_millis();
    let peak = std::fs::read_to_string("/proc/self/status").unwrap_or_default();
    let peak = peak.lines().find(|l| l.starts_with("VmHWM:")).unwrap_or("VmHWM: unavailable");
