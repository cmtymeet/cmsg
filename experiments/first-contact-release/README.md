...
`restorePending` seals and restores the pending record with a child-owned random wrapping key and expected context. No snapshot or wrapping key is output. This is an in-process encrypted restore, not process-crash recovery, freshness protection or a production wallet. The three composition cases still await their first passing run; the separately passing fifteen isolated and 83 core contracts are not evidence that this new process composition succeeds.
