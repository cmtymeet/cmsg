...
Separate unpublished Rust experiment for the [release-gate design](../../studies/first-contact-release-gate.md). Current source is a fail-first contract with explicit stubs; no runtime result is claimed yet.
...
The proposed owned pending state holds the exact withheld Welcome/ciphertext, current context, sender and expected recipient IDs/keys, a fresh private release nonce, a separate sender authorization nonce and one bound blind request hash. Release requires both independently signed operator attestations to match. Its encrypted restore is intended to authenticate saved state, not establish freshness, atomic host durability or protection from rollback/concurrent copies.
...
No local compilation or package publication is needed. Runtime red must precede replacing the stubs.
