...
- Renewal persistence currently proves local rollback on failed writes, not recovery of a lost outbound commit after a crash. The callback must gain the exact control bytes alongside candidate state so the host can atomically encrypt and save an outbox entry. Low-level add/remove delivery and persistence remain host-managed.
